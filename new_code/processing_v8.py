...
def aggregate_glare_results(df_glare_results):
    ...
    df_aggregated = df_glare_results.groupby(['OP Number', 'PV Area Name', 'timestamp'], as_index=False).apply(calculate_extents, ).reset_index(drop=True)
...
def calculate_glare(_pv_areas, list_of_pv_area_information, list_of_ops, _meta_data, _simulation_parameter, api_key, output_dir, excluded_areas=[]):
    ...
    # Step 1: Metadata and Parameter Initialization
    ...
    print_duration("Loading module data and fitting polynomial", total_start_time)

    # Step 2: Initialize PV Areas and Observation Points
    pv_areas = []
    for i, pvs in enumerate(_pv_areas):
        points = [MyPoint(point['latitude'], point['longitude'], point['ground_elevation'], point['height_above_ground']) for point in pvs]
    ...
    list_of_dps = [MyPoint(op['latitude'], op['longitude'], op['ground_elevation'], op['height_above_ground']) for op in list_of_ops]
    initialize_global_transformer(list_of_dps[0].latitude, list_of_dps[0].longitude)

    # Step 3: Preprocess PV Areas
    ...
    print_duration("Preprocessing PV areas", total_start_time)

    # Step 4: Calculate Angles and Distances
    data = []
    for original_area in pv_areas:
    ...
    print_duration("Calculating angles and distances", total_start_time)

    # Step 5: Generate Calculation Points and Sun Data
    ...
    print_duration("Generating calculation points and sun positions", total_start_time)

    # Step 6: Reflection and Glare Calculations
    ...
    def run_plot_glare_data():
        ...
        print_duration("Plotting glare data", total_start_time)
        update_status(0.52, _meta_data['sim_id'])

    def run_plot_glare_intensity():
        plot_glare_intensity_with_continuous_colorbar(df_aggregated, output_dir, meta_data.timestamp, list_of_dps, meta_data.utc)
        print_duration("Plotting glare intensity", total_start_time)

    # Create Threads for Each Function
    threads = [
        threading.Thread(target=run_generate_static_map),
        threading.Thread(target=run_generate_and_save_plots),
        threading.Thread(target=run_plot_glare_data),
        threading.Thread(target=run_plot_glare_intensity)
    ]
    ...
    # Step 7: Generate Final Report After Threads Complete
    generate_reports(df_aggregated, output_dir, list_of_dps, meta_data, simulation_parameter, pv_areas)
    print_duration("Generating reports", total_start_time)
